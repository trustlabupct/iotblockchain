...
class Statistics:
    ...
    @staticmethod
    def record_transaction_time(tx_id, creation_time, inclusion_time):
        # Verificar si la transacción ya ha sido registrada
        if not any(tx['tx_id'] == tx_id for tx in Statistics.transaction_times):
            Statistics.transaction_times.append({
                'tx_id': tx_id,
                'creation_time': creation_time,
                'inclusion_time': inclusion_time,
                'time_to_include': inclusion_time - creation_time
            })

    @staticmethod
    def print_transaction_times_to_excel(fname):
        df = pd.DataFrame(Statistics.transaction_times)
        writer = pd.ExcelWriter(fname, engine='xlsxwriter')
        df.to_excel(writer, sheet_name='TransactionTimes')
        writer._save()
