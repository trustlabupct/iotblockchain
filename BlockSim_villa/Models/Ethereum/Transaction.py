...
import operator
import copy
from Statistics import Statistics  # Asegúrate de importar Statistics
...
class LightTransaction:
    ...
    def execute_transactions():
        ...
        size = 0  # calculate the total block gas limit
        count = 0
        blocksize = p.Bsize
        pool = LightTransaction.pending_transactions
...
class FullTransaction:
    ...
    @staticmethod
    def create_transactions():
        Psize = int(p.Tn * p.simTime)  # Total number of transactions
        interarrival_times = np.random.exponential(1.0 / p.Tn, Psize)
        arrival_time = 0
        print(f"Se van a generar {Psize} transacciones")
        for i in range(Psize):
            tx = Transaction()
            tx.id = random.randrange(100000000000) + i  # Asegurar IDs únicos
            arrival_time += interarrival_times[i]
            creation_time = int(arrival_time)
    ...
    def transaction_prop(tx):
        ...
        for i in p.NODES:
            if tx.sender != i.id:
                t = copy.deepcopy(tx)
                t.timestamp[1] = t.timestamp[1]  # + Network.tx_prop_delay()  # transaction propagation delay in seconds
    ...
    def execute_transactions(miner, currentTime):
        ...
        # Registrar el tiempo de inclusión de cada transacción
        for tx in transactions:
            Statistics.record_transaction_time(tx.id, tx.timestamp[0], currentTime)
