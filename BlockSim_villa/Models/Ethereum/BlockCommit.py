...
class BlockCommit(BaseBlockCommit):
        ...
        if blockPrev == miner.last_block().id:
            ...
            if p.hasTrans:
                ...
                for tx in event.block.transactions:
                    if len(tx.timestamp) < 2:
                        tx.timestamp = [0, eventTime]
                    else:
                        tx.timestamp[1] = eventTime  # Registrar el tiempo de inclusión
                    Statistics.record_transaction_time(tx.id, tx.timestamp[0], eventTime)
    ...
    def receive_block(event):
        ...
        # case 2: the received block is not built on top of the last block
            ...
            # if depth of the received block <= depth of the last block, then reject the block (add it to unclechain)
            else:
                uncle = event.block
